...
const SecondsForRound = 12000;
const SecondsForShowScore = 9000;
...
io.on("connection", (socket: Socket) => {
  ...
  const startNextRound = (room: Room) => {
    try {
      ...
      if (room.round >= room.rounds || room.players.length < 2) {
        ...
        console.log(
          `[END GAME CHECK] Round: ${room.round}/${room.rounds}, Players: ${room.players.length}, Planted photos left: ${playersWithPlantedPhotos.length}`
        );
      ...
      if (plantedPhotosCount > 0) {
        ...
        if (remainingRounds <= plantedPhotosCount + 1) {
          ...
          selectionReason = `End game approach (${remainingRounds} rounds left, ${plantedPhotosCount} photos). Random chance: ${randomChance.toFixed(
            2
          )} vs threshold 0.7`;
        } else {
          ...
          const nearPositions = plantedPhotoPositions.filter((pos) => Math.abs(roundPosition - pos) < 0.8 / totalRounds);
      ...
      if (shouldShowPlantedPhoto && playersWithPlantedPhotos.length > 0 && room.round < room.rounds) {
        ...
        console.log(
          `[PLANTED PHOTO SELECTED] From ${selectedPlayer.username} (${room.plantedPhotosShown} planted photos shown so far)`
        );
  ...
  socket.on("chat-message", (data: { gameCode: string; username: string; message: string }) => {
    try {
      const { gameCode, username, message } = data;
      console.log(`Chat message from ${username} in room ${gameCode}: ${message}`);

      // Validate message length
      if (message.trim() && message.length <= 100) {
        // Broadcast message to all other clients in the room
        socket.broadcast.to(gameCode).emit("chat-message", {
          username,
          message,
        });
      }
